...
def prepare_clustered_dataset(
    input_path="data/clustered_dataset_rh.csv",
    ...
    if "RH" not in df.columns:
        ...
        temp_cols = [
            col for col in ["T_air", "T2M", "Temperature"] if col in df.columns
        ]
        dew_cols = [
            col for col in ["Dew_Point", "TD2M", "Dewpoint", "d2m"] if col in df.columns
        ]
...
def run_kmedoids_clustering(
    X_scaled, n_clusters=5, metric="euclidean", random_state=42
):
    kmedoids = KMedoids(
...
def find_optimal_k(X_scaled, k_range=range(2, 11), random_state=42):
    ...
    for k in k_range:
        try:
            model = KMedoids(
                n_clusters=k, init="k-medoids++", random_state=random_state
            )
            model.fit(X_scaled)
...
def compute_pv_potential_by_cluster_year(
    ...
    if year_col not in df.columns:
        if "time" in df.columns:
            ...
            raise ValueError(
                f"Column '{year_col}' not found and no 'time' column to derive from."
            )

    grouped = (
        df.groupby([cluster_col, year_col])
        .agg(
...
def match_technology_to_clusters(cluster_spectra_df, pv_profiles, temp_col="T_air"):
    ...
    for _, row in cluster_spectra_df.iterrows():
        ...
        for tech, profile in pv_profiles.items():
            ...
            temp_penalty = temp_coeff * (
                cluster_temp - 25
            )  # assume PR loss below/above 25°C
...
def compute_cluster_summary(
    df, cluster_col="Cluster_ID", output_path="results/cluster_summary.csv"
):
    """
...
def plot_prediction_uncertainty_with_contours(
    ...
    if use_hatching or contour_levels:
        ...
        if use_hatching:
            ...
            cs = ax.contour(
                xi, yi, zi, levels=contour_levels, colors="black", linewidths=0.7
            )
            ax.clabel(
                cs,
                inline=1,
...
def plot_technology_matches(
    ...
    df_merged = df_clustered.merge(
        match_df[[cluster_col, "Best_Technology"]], on=cluster_col, how="left"
    )
...
def plot_clusters_with_kg(
    ...
    lon_col="longitude",
    cluster_col="Cluster_ID",
    output_path="figures/cluster_map_with_kg.png",
):
...
def main_matching_pipeline(
    ...
    df_final = df_clustered.merge(
        match_df[["Cluster_ID", "Best_Technology"]], on="Cluster_ID", how="left"
    )
    ...
    df_final = df_final.merge(
        df_clustered[["Cluster_ID", "Cluster_Label"]], on="Cluster_ID", how="left"
    )

    print("\n=== Plotting Final Technology Recommendation Map ===")
    plot_technology_matches(
        df_final, match_df, cluster_col="Cluster_ID", borders_path=shapefile_path
    )
    plot_prediction_uncertainty(
        df_final, output_path="results/maps/prediction_uncertainty_map.png"
    )
