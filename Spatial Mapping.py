...
def add_land_mask(
    df, lat_col="latitude", lon_col="longitude", world_shapefile="naturalearth_lowres"
):
    """
...
def overlay_technology_matches(
    ...
    # Web Mercator for plotting with basemaps
...
def export_geojson(
    df,
    output_path="results/maps/final_clustered_map.geojson",
    lat_col="latitude",
    lon_col="longitude",
    crs_epsg=4326,
):
...
def calculate_pv_potential(GHI, T_air, RC_potential, Red_band, Total_band):
    ...
    if np.any(RC_potential < -100) or np.any(RC_potential > 300):
        print(
            "⚠️ Warning: RC potential values outside expected range (-100 to 300 W/m²)"
        )
...
def predict_pv_potential(model, X_scaled, df_original):
    ...
    # Optional: Add prediction confidence if model supports it (Random Forest
    # does)
    try:
        pred_std = np.std(
            [tree.predict(X_scaled) for tree in model.estimators_], axis=0
        )
...
def run_kmedoids_clustering(
    X_scaled, n_clusters=4, metric="euclidean", random_state=42
):
    kmedoids = KMedoids(
...
def plot_technology_matches(
    ...
    df_merged = df_clustered.merge(
        match_df[["Cluster_ID", "Best_Technology"]], on=cluster_col, how="left"
    )
    gdf = gpd.GeoDataFrame(
        df_merged,
...
def main_clustering_pipeline(
    input_file="merged_dataset.csv", output_file="clustered_dataset.csv", n_clusters=5
):
    ...
    model, X_train, X_test, y_train, y_test, y_pred = train_random_forest(
    ...
    correlation = np.corrcoef(
        df_with_pred["PV_Potential_physics"], df_with_pred["Predicted_PV_Potential"]
    )[0, 1]
    print(f"Correlation between physics and ML predictions: {correlation:.4f}")

    print("Preparing features for clustering...")
    X_cluster_scaled, cluster_feats, scaler_cluster = prepare_features_for_clustering(
        df_with_pred
    )

    print("Running K-Medoids clustering...")
    kmedoids, labels, silhouette = run_kmedoids_clustering(
        X_cluster_scaled, n_clusters=n_clusters
    )
...
def plot_prediction_uncertainty(
    ...
    lat_col="latitude",
    lon_col="longitude",
    output_path="results/maps/prediction_uncertainty_map.png",
):
    """
...
def plot_overlay_rc_pv_zones(
    ...
    rc_col="RC_Cluster",
    tech_col="Best_Technology",
    output_path="results/maps/rc_pv_overlay.png",
):
    import contextily as ctx
    import geopandas as gpd
    import matplotlib.pyplot as plt
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", default="merged_dataset.csv", help="Input CSV file path"
    )
    parser.add_argument(
        "--output", default="clustered_dataset.csv", help="Output CSV file path"
    )
    parser.add_argument(
        "--clusters", type=int, default=5, help="Number of clusters for K-Medoids"
    )
    parser.add_argument(
        "--landmask", action="store_true", help="Apply land mask and export GeoJSON"
    )
    ...
    if args.landmask:
        ...
        land_df.to_file(
            "results/clusters/clustered_land_only.geojson", driver="GeoJSON"
        )

    # Main pipeline
    final_df = main_clustering_pipeline(
        input_file=args.input, output_file=args.output, n_clusters=args.clusters
    )
