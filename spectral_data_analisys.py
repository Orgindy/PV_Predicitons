    ...
    for ext_file in ext_files:
        ...
        try:
            ...
            try:
                ...
                logging.info(f"⚠️ Could not parse lat/lon from filename {ext_file}. Skipping.")
                continue
            ...
            band_ranges = [(0.3, 0.4), (0.4, 0.5), (0.5, 0.6), (0.6, 0.7), (0.7, 1.0), (1.0, 2.5), (2.5, 50.0)]
...
def combine_band_data(input_folder, output_file):
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            required_columns = ["latitude", "longitude", "UV", "Blue", "Green", "Red", "NIR", "SWIR", "FIR"]
...
def add_spectral_ratios(input_file, output_file):
    ...
    df['Total_Irradiance'] = df[['UV', 'Blue', 'Green', 'Red', 'NIR', 'SWIR', 'FIR']].sum(axis=1)
...
def calculate_band_metrics(df, wavelength_col='Wvlgth', irradiance_cols=None,
                           bands=None):
    """
    ...
    if irradiance_cols is None:
        ...
        if not irradiance_cols:
            ...
            irradiance_cols = [col for col in df.columns if any(
                term in col.lower() for term in ['irrad', 'tilt', 'beam', 'difuse', 'global', 'horiz'])]
    ...
    for location_id in df['location_id'].unique() if 'location_id' in df.columns else [None]:
        for date_time in df['date_time'].unique() if 'date_time' in df.columns else [None]:
            # Filter data if needed
            if location_id is not None and date_time is not None:
                filtered_df = df[(df['location_id'] == location_id) & (df['date_time'] == date_time)]
            ...
            for irradiance_col in irradiance_cols:
                for band_name, (band_min, band_max) in bands.items():
                    ...
                    band_df = filtered_df[(filtered_df[wavelength_col] >= band_min) & 
                    ...
                    try:
                        ...
                        band_irradiance = integrate.trapz(
                            band_df[irradiance_col], 
                            band_df[wavelength_col]
                        )
                        
                        # Calculate normalized metrics
                        if band_name != 'Total':
                            total_df = filtered_df[(filtered_df[wavelength_col] >= 280) & 
                                                   (filtered_df[wavelength_col] <= 4000)]
                            if len(total_df) >= 2:
                                total_df = total_df.sort_values(by=wavelength_col)
                                total_irradiance = integrate.trapz(
                                    total_df[irradiance_col], 
                                    total_df[wavelength_col]
                                )
                                normalized = band_irradiance / total_irradiance if total_irradiance > 0 else 0
                            else:
                                total_irradiance = 0
                                normalized = 0
                        else:
                        ...
                        logging.info(f"Error calculating {band_name} for {irradiance_col}: {e}")
    
    return pd.DataFrame(results)

def plot_spectral_irradiance(df, wavelength_col='Wvlgth', irradiance_cols=None, 
                            title=None, log_scale=False, show_bands=True):
    """
    ...
    if show_bands:
        ...
        for band_name, ((band_min, band_max), color) in bands.items():
            if band_min >= min(df[wavelength_col]) and band_max <= max(df[wavelength_col]):
    ...
    if title:
        ...
        location = df['location_id'].iloc[0] if 'location_id' in df.columns else 'Unknown'
...
def create_spectral_composition_plots(band_df, output_dir):
    ...
    for irradiance_type in band_df['irradiance_type'].unique():
        ...
        df_filt = band_df[(band_df['irradiance_type'] == irradiance_type) & 
        ...
        for location in locations:
            ...
            for date_time in datetimes:
                ...
                for band in bands:
                    band_value = df_time[df_time['band_name'] == band]['normalized_fraction'].values
            ...
            for band in bands:
                if band in plot_df.columns:
                    color = band_colors.get(band, None)  # Use predefined color or let matplotlib choose
                    ax.bar(plot_df['date_time'], plot_df[band], bottom=bottom, label=band, color=color)
...
def plot_location_comparison(band_df, band_name, irradiance_type, output_dir, 
                           time_points=None, normalized=True):
    """
    ...
    # Filter data
    df_filt = band_df[(band_df['band_name'] == band_name) & 
    ...
    if normalized:
        pivot = df_filt.pivot(index='location_id', columns='date_time', values='normalized_fraction')
        value_label = 'Normalized Fraction'
    else:
        pivot = df_filt.pivot(index='location_id', columns='date_time', values='irradiance_W_m2')
...
def plot_temporal_variation(band_df, location_id, band_name, output_dir):
    ...
    fig, axes = plt.subplots(len(irradiance_types), 1, figsize=(12, 4 * len(irradiance_types)), sharex=True)
    ...
    for i, irradiance_type in enumerate(irradiance_types):
        ...
        for band in bands:
            ...
            ax.plot(df_band['date_time'], df_band['irradiance_W_m2'], 
                   marker='o', label=f"{band}")
    ...
    for irradiance_type in band_df['irradiance_type'].unique():
        ...
        for loc in df_irr['location_id'].unique():
            for dt in df_irr[df_irr['location_id'] == loc]['date_time'].unique():
                df_lt = df_irr[(df_irr['location_id'] == loc) & (df_irr['date_time'] == dt)]
                ...
                for band1, band2, ratio_name in ratios:
                    # Get band values
                    band1_val = df_lt[df_lt['band_name'] == band1]['irradiance_W_m2'].values
                    band2_val = df_lt[df_lt['band_name'] == band2]['irradiance_W_m2'].values
        ...
        for _, _, ratio_name in ratios:
            ...
            if len(ratio_df['location_id'].unique()) > 1 and location_id is None:
                ...
                ax.bar(ratio_df['location_id'] if location_id is None else ratio_df['date_time'], 
                      ratio_df[ratio_name])
...
def main():
    # HARD-CODED PATHS
    input_folder = "./smarts_out_files"  # <- Change to your folder path
    output_folder = "./spectral_analysis_output"
    ...
    # Save the metrics
    metrics_file = os.path.join(output_folder, "band_metrics.csv")
    band_df.to_csv(metrics_file, index=False)
    logging.info(f"💾 Saved band metrics to {metrics_file}")
