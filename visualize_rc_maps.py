...
def create_yearly_maps(yearly_df, output_dir, boundaries):
    ...
    ax1.set_title(
        f"Basic Radiative Cooling Potential (P_rc_basic) - {latest_year}", fontsize=12
    )
    ...
    # Add title and colorbar
    ax2.set_title(
        f"Net Radiative Cooling Potential (P_rc_net) - {latest_year}", fontsize=12
    )
...
def create_seasonal_maps(seasonal_df, output_dir, boundaries, variable):
    ...
    if variable not in df_year.columns:
        print(
            f"Warning: Variable {variable} not found in data. Available variables: {df_year.columns}"
        )
        return
    ...
    plt.suptitle(
        f"Seasonal {variable_label} Radiative Cooling Potential for Europe - {latest_year}",
        fontsize=16,
        y=0.98,
    )
    ...
    output_file = os.path.join(
        output_dir, f"seasonal_rc_potential_{variable_str}_{latest_year}.png"
    )
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    if len(sys.argv) != 4:
        print(
            "Usage: python visualize_rc_maps.py yearly_file.csv seasonal_file.csv output_directory"
        )
        sys.exit(1)
    ...
    print(
        "Map boundaries: Longitude"
        f" [{boundaries[0]:.2f}, {boundaries[1]:.2f}],"
