...
DEFAULT_RHO = (
    # Default solar reflectivity (used if GRIB effective albedo not available)
    0.2
)
...
# Options: 'spherical', 'exponential', 'gaussian', etc.
...
# Path to your CSV files folder (containing the NASA CSV files with
# climate data for RC cooling)
...
def get_effective_albedo(
    grib_path: str, timestamp: pd.Timestamp, lat: float, lon: float
) -> float:
    """
    ...
    if not grib_files:
        logging.warning(
            f"No GRIB file found for {year}-{month:02d}; using default albedo."
        )
        return DEFAULT_RHO
...
def calculate_qnet_vectorized(
    df: pd.DataFrame,
    sigma: float = STEFAN_BOLTZMANN,
    eps_coat: float = DEFAULT_EPS_COAT,
) -> pd.Series:
    """
...
def add_effective_albedo_optimized(chunk, grib_path):
    ...
    for time_val in unique_times:
        try:
            ...
            pattern = os.path.join(
                grib_path, f"*{time_val.year}{time_val.month:02d}*.grib"
            )
            grib_files = glob.glob(pattern)
            if grib_files:
                with xr.open_dataset(grib_files[0], engine="cfgrib") as ds:
                    if "fal" in ds:
                        albedo_cache[time_val] = (
                            ds["fal"].sel(time=time_val, method="nearest").load()
                        )
        except Exception as e:
    ...
    def get_albedo(row):
        time_key = (
            pd.to_datetime(row["time"]).floor("H") if pd.notnull(row["time"]) else None
        )
...
def load_and_process_csv_files(
    folder_path: str, chunk_size: int = CHUNK_SIZE, grib_path: str = GRIB_PATH
) -> pd.DataFrame:
    """
    ...
    possible_columns = [
        ...
        "T2M",
        "ALLSKY_SFC_LW_DWN",
        "ALLSKY_SFC_SW_DWN",  # for QNET calculation
        ...
        for chunk in pd.read_csv(file, chunksize=chunk_size):
            ...
            for col in [
                ...
                if col in chunk.columns:
                    chunk[col] = pd.to_numeric(
                        chunk[col], errors="coerce", downcast="float"
                    )
            ...
            if "time" in chunk.columns:
                ...
                try:
                    ...
                    logging.error(
                        "Error constructing time column from YEAR, MO, DY, HR",
                        exc_info=True,
                    )
                    chunk["time"] = pd.NaT
            else:
            ...
            # If GRIB path is provided, add effective_albedo column using
            # row-wise lookup.
            if grib_path:
...
def add_cluster_labels(
    ...
    rc_gdf = gpd.GeoDataFrame(
        rc_df,
        geometry=gpd.points_from_xy(rc_df[lon_col], rc_df[lat_col]),
        crs="EPSG:4326",
    )
    cluster_gdf = gpd.GeoDataFrame(
        cluster_df,
        geometry=gpd.points_from_xy(cluster_df[lon_col], cluster_df[lat_col]),
        crs="EPSG:4326",
    )
    ...
    rc_gdf_with_labels = gpd.sjoin(
        rc_gdf,
        cluster_gdf[[cluster_col, "geometry"]],
        how="left",
...
def load_and_merge_rc_netcdf_years(folder_path=None, var_name="QNET", time_dim="time"):
    ...
    for file in files:
        ...
        with xr.open_dataset(file) as ds:
            ...
            # Load data into memory, then close file
...
def multi_year_kriging(
    ...
    if cluster_file:
        ...
        df = pd.merge(
            df,
            cluster_df[[lat_col, lon_col, "Cluster_ID"]],
            on=[lat_col, lon_col],
            how="left",
        )
    ...
    if "Cluster_ID" in df.columns:
        ...
        for cluster_id in df["Cluster_ID"].unique():
            ...
            try:
                ...
                # Predict on the same points (could be a grid for higher
                # resolution)
...
def aggregate_rc_metrics(kriged_file, output_file, cluster_col="Cluster_ID"):
    ...
    metrics_df = (
        ...
        .agg({"RC_Kriged": ["mean", "median", "std", "min", "max", "sum", "count"]})
        .reset_index()
    )

    # Flatten multi-level columns
    metrics_df.columns = [
        f"{col[0]}_{col[1]}" if col[1] else col[0] for col in metrics_df.columns
    ]
...
def estimate_sky_temperature_hybrid(df):
    ...
    # Now imported from humidity.py
    RH = compute_relative_humidity(T_air_K, T_dew_K)
...
def calculate_day_night_rc_power(
    df, albedo=0.3, emissivity=0.95, zenith_col="solar_zenith"
):
    """
    ...
    try:
        ...
        logging.warning(
            f"Hybrid sky temperature estimate failed: {e}. Falling back to default."
        )
...
def evaluate_model(
    OK: OrdinaryKriging, X_test: pd.DataFrame, y_test: pd.Series
) -> None:
    """Evaluate the kriging model on the test set and log the RMSE."""
    pred, _ = OK.execute(
        "points", X_test["LON"].values.astype(float), X_test["LAT"].values.astype(float)
    )
    pred = np.array(pred)
...
def plot_rc_split_maps(
    df,
    lat_col="LAT",
    lon_col="LON",
    rc_day_col="RC_day",
    rc_night_col="RC_night",
    save_day="rc_day_map.png",
    save_night="rc_night_map.png",
):
    """
...
def plot_qnet_map(
    ...
    ax.add_feature(cfeature.BORDERS, linewidth=0.5)
    ax.set_extent(
        [GRID_LON_RANGE[0], GRID_LON_RANGE[1], GRID_LAT_RANGE[0], GRID_LAT_RANGE[1]],
        crs=ccrs.PlateCarree(),
    )
    ...
    ax.set_title(
        "Annual Radiative Cooling Potential (kWh/m²·year)",
        fontsize=14,
        fontweight="bold",
    )
...
def main(db_url=None, db_table="rc_data"):
    try:
        ...
        # Optionally, filter to a single year (e.g., annual_df =
        # annual_df[annual_df['year'] == 2019])
        ...
        # Optionally specify a save path for the map, e.g.,
        # "rc_potential_map.png"
