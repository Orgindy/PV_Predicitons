...
def auto_generate_physics_pv(input_file, output_file, temp_coeff=-0.0045):
    ...
    # 5. Corrected PR
    df["PR_corrected"] = (
        PR_ref + df["Temp_Loss"] + df["RC_Gain"] + df["Spectral_Adjust"]
    )
    df["PR_corrected"] = np.clip(
        df["PR_corrected"], 0.7, 0.9
...
def main_csv_workflow(input_file, validated_file, physics_file, results_dir):
    ...
    print("Added physics-based PV potential")
    ...
    plt.close()
    print(
        f"📊 Feature importance plot saved to {os.path.join(results_dir, 'feature_importance_plot.png')}"
    )
...
def main_netcdf_workflow(netcdf_file, results_dir):
    ...
    df = load_netcdf_data(netcdf_file, sample_fraction=0.1)
...
def main():
    ...
    if args.db_url:
        ...
        main_csv_workflow(
            temp_path, args.validated_file, args.physics_file, args.results_dir
        )
        final_csv = os.path.join(args.results_dir, "merged_with_physics_pv.csv")
        if os.path.exists(final_csv):
            processed = pd.read_csv(final_csv)
            write_dataframe(
                processed, args.db_table, db_url=args.db_url, if_exists="replace"
            )
        ...
        main_csv_workflow(
            args.input_file, args.validated_file, args.physics_file, args.results_dir
        )
    else:
