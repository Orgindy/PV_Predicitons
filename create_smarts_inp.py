...
Examples:
    ...
    python smarts_era5_generator_eu.py --input era5_data.nc --output ./smarts_inputs \
        --elevation-file elevation_summary.csv
...
EU_BOUNDS = {"lat_min": 40, "lat_max": 70.0, "lon_min": -30.0, "lon_max": 40.0}

# ------------------ ERA5 Variable Mapping ------------------
# Here we map ERA5 variable names to internal SMARTS keys and provide
...
ERA5_VARIABLE_MAP = {
    ...
    "tcwv": (
        "precipitable_water",
        lambda x: x / 10,
    ),  # Total column water vapor: kg/m² → cm
    # Skin temperature: K → °C
    "skt": ("skin_temperature", lambda x: x - 273.15),
    "tcc": ("cloud_cover", lambda x: x * 100),  # Total cloud cover: (0–1) → %
    # Surface solar radiation down
    "ssrd": ("solar_radiation_down", lambda x: x),
    "strd": (
        "thermal_radiation_down",
        lambda x: x,
    ...
    "aluvp": (
        "uv_albedo_direct",
        lambda x: x * 100,
    ),  # UV visible albedo direct: (0–1) → %
    "aluvd": (
        "uv_albedo_diffuse",
        lambda x: x * 100,
    ),  # UV visible albedo diffuse: (0–1) → %
    "alnip": (
        "nir_albedo_direct",
        lambda x: x * 100,
    ),  # Near IR albedo direct: (0–1) → %
    "alnid": (
        "nir_albedo_diffuse",
        lambda x: x * 100,
    ),  # Near IR albedo diffuse: (0–1) → %
    # High cloud cover: (0–1) → %
    "hcc": ("high_cloud_cover", lambda x: x * 100),
    # Medium cloud cover: (0–1) → %
    "mcc": ("medium_cloud_cover", lambda x: x * 100),
    "lcc": ("low_cloud_cover", lambda x: x * 100),  # Low cloud cover: (0–1) → %
    "ssrdc": ("clear_sky_solar_rad", lambda x: x),  # Clear-sky solar rad
    # Clear-sky direct solar rad
    "cdir": ("clear_sky_direct_rad", lambda x: x),
    "fdir": ("direct_solar_rad", lambda x: x),  # Direct solar radiation
    "duvr": ("uv_radiation_down", lambda x: x),  # Downward UV radiation
    # Cloud water (units assumed consistent)
    "tcdw": ("cloud_water", lambda x: x),
    "tp": ("precipitation", lambda x: x * 1000),  # Total precipitation: m → mm
    # Maximum temperature: K → °C
    "mx2t": ("max_temperature", lambda x: x - 273.15),
    # Minimum temperature: K → °C
    "mn2t": ("min_temperature", lambda x: x - 273.15),
    "u10": ("u_wind", lambda x: x),  # 10m u-component
    "v10": ("v_wind", lambda x: x),  # 10m v-component
    # TOA incident solar radiation
    "tisr": ("toa_incident_solar_rad", lambda x: x),
    # Total column rain water: m → mm
    "tcrw": ("rain_water", lambda x: x * 1000),
    # Evaporation: m → mm (if applicable)
    "e": ("evaporation", lambda x: x * 1000),
    "ptype": (
        "precipitation_type",
        lambda x: x,
    ),  # Precipitation type: categorical/string
    ...
    "ssrc": (
        "net_solar_radiation_clear",
        lambda x: x,
    ),  # Surface net solar radiation, clear sky
    # Surface net thermal radiation
    "str": ("net_thermal_radiation", lambda x: x),
    ...
    "ttr": ("top_net_thermal_radiation", lambda x: x),
    "ttrc": (
        "top_net_thermal_radiation_clear",
        lambda x: x,
    ),  # Top net thermal radiation, clear sky
    "strdc": (
        "thermal_radiation_down_clear",
        lambda x: x,
...
def select_representative_days(
    input_file, output_file, method="solstice", sample_days=4
):
    """
...
def extract_era5_values(ds, lat, lon, time, stats=None):
    ...
    for var, (key, transform) in ERA5_VARIABLE_MAP.items():
        try:
            ...
            if stats is not None:
                stats["missing_critical"][var] = (
                    stats.get("missing_critical", {}).get(var, 0) + 1
                )
            point_data[key] = None
        except Exception:
            if stats is not None:
                stats["extraction_errors"][var] = (
                    stats.get("extraction_errors", {}).get(var, 0) + 1
                )
                stats["total_extraction_errors"] = (
                    stats.get("total_extraction_errors", 0) + 1
                )
            point_data[key] = None
...
def calculate_cloud_effects(params):
    ...
    high_transmittance = 1.0 - (
        high_cloud / 100 * 0.3
    )  # High clouds block ~30% of direct
    # Mid clouds block ~50% of direct
    mid_transmittance = 1.0 - (mid_cloud / 100 * 0.5)
    # Low clouds block ~80% of direct
    low_transmittance = 1.0 - (low_cloud / 100 * 0.8)
    ...
    return {
        ...
        "diffuse_increase": 1.0
        + (1.0 - direct_transmittance)
        * 0.5,  # Portion of blocked direct becomes diffuse
    }
...
def determine_advanced_aerosol_type(params):
    ...
    rh = params.get("rh", 50)
    params.get("wind_speed") or 0.0
    precipitation = params.get("precipitation") or 0.0

    # Maritime conditions
    if land_cover == "WATER" or (
        elevation < 100
        and (
            abs(lon - 14.0) < 5.0
            or abs(lon - 20.0) < 5.0
...
def calculate_enhanced_albedo(params):
    ...
    has_all_albedo_components = all(
        x in params and params[x] is not None
        for x in [
            "uv_albedo_direct",
            "uv_albedo_diffuse",
            "nir_albedo_direct",
    ...
    if has_all_albedo_components:
        ...
        uv_albedo = params["uv_albedo_diffuse"] * diffuse_fraction + params[
            "uv_albedo_direct"
        ] * (1 - diffuse_fraction)
        nir_albedo = params["nir_albedo_diffuse"] * diffuse_fraction + params[
            "nir_albedo_direct"
        ] * (1 - diffuse_fraction)
        ...
        if "snow_albedo" in params and params["snow_albedo"] is not None:
            ...
            if snow_cover > 0.05:  # More than 5% snow cover
                return snow_cover * params["snow_albedo"] + (1 - snow_cover) * (
                    0.4 * uv_albedo + 0.6 * nir_albedo
                )
        if "snow_cover" not in params:
            if (
                params.get("temperature", 5) < 0
                and params.get("precipitation", 0) > 2.0
            ):
                params["snow_cover"] = 0.5  # Assume partial snow cover

        if (
            "snow_cover" in params
            and "snow_albedo" in params
            and params["snow_albedo"] is not None
        ):
            snow_cover = min(1.0, params["snow_cover"])
            albedo = snow_cover * params["snow_albedo"] + (1 - snow_cover) * (
                0.4 * uv_albedo + 0.6 * nir_albedo
            )
...
def add_elevation_and_pressure(input_file, elevation_file, output_file):
    ...
    if merged_df["elevation"].isna().sum() > 0:
        print(
            "❌ Missing elevation data for some locations. Check your elevation file."
        )
        return

    # Calculate pressure at altitude (barometric formula)
    # P = P0 * exp(-g * M * h / (R * T))
    # Assuming P0 = 1013.25 hPa (sea level), T = 288.15 K, M = 0.029 kg/mol, g
    # = 9.81 m/s², R = 8.314 J/(mol·K)
    P0 = 1013.25  # hPa
    M = 0.029  # kg/mol
    g = 9.81  # m/s²
    R = 8.314  # J/(mol·K)
...
def validate_param(name, value, stats=None):
    ...
    if (
        ...
        if stats is not None and name in defaults:
            stats["missing_params"][name] = (
                stats.get("missing_params", {}).get(name, 0) + 1
            )
    ...
    if name in ranges and (value < ranges[name][0] or value > ranges[name][1]):
        if stats is not None:
            stats["invalid_params"][name] = (
                stats.get("invalid_params", {}).get(name, 0) + 1
            )

        # Clamp to range rather than reset to default
        if value < ranges[name][0]:
            logger.warning(
                f"⚠️ {name} value {value} below range minimum, clamping to {ranges[name][0]}"
            )
            return ranges[name][0]
        else:
            logger.warning(
                f"⚠️ {name} value {value} above range maximum, clamping to {ranges[name][1]}"
            )
...
def validate_era5_dataset(ds):
    ...
    logger.info(
        f"ERA5 dataset validated successfully. Contains {len(ds.data_vars)} variables."
    )
    return True
...
def calculate_solar_position(lat, lon, time):
    ...
    declination = (
        ...
        + 0.001480 * math.sin(3 * angle)
    )

    # More accurate equation of time
    eot = 229.18 * (
        0.000075
        + 0.001868 * math.cos(angle)
        - 0.032077 * math.sin(angle)
        - 0.014615 * math.cos(2 * angle)
    ...
    if solar_zenith > 90:
        logger.debug(
            f"Sun below horizon at lat={lat}, lon={lon}, time={time}. Zenith = {solar_zenith:.2f}°"
        )
    ...
    y = math.cos(hour_angle_rad) * math.sin(lat_rad) * math.cos(decl_rad) - math.sin(
        decl_rad
    ) * math.cos(lat_rad)
...
def determine_season(month, lat):
    ...
    if lat >= 0:
        if 5 <= month <= 8:  # May through August
            ...
            return (
                "SUMMER" if lat < 50 else "WINTER"
            )  # Higher latitudes have longer winters
    # Southern Hemisphere (not really relevant for EU but included for
    # completeness)
    else:
...
def run_unit_tests():
    ...
    class TestSmartsGenerator(unittest.TestCase):
        ...
        def test_calculate_solar_position(self):
            ...
            # Approximate check - exact values would need a reference
            # calculation
            # Low solar zenith at summer solstice
            self.assertTrue(25 <= zenith <= 35)
        ...
        def test_estimate_aod_era5_only(self):
            ...
            self.assertTrue(
                0.25 <= aod <= 0.4, f"Urban AOD {aod} outside expected range"
            )
            ...
            aod = estimate_aod_era5_only(params)
            self.assertTrue(
                0.05 <= aod <= 0.15, f"Maritime AOD {aod} outside expected range"
            )
...
def create_smarts_input_header(params):
    ...
    time_str = (
        params["time"].strftime("%Y%m%d_%H%M")
        if "time" in params
    ...
    content.append("1 !Card 3 IATMOS")
    content.append(f"'{ref_atmosphere}' !Card 3a Atmos")

    # Card 4: Water vapor
    if "precipitable_water" in params and params["precipitable_water"] is not None:
        precip_water = validate_param(
            "precipitable_water", params["precipitable_water"]
        )
        content.append("1 !Card 4 IH2O")
...
def create_smarts_input_surface(params):
    ...
    content.append(
        f"{params['tilt_angle']} {params['azimuth']} {int(round(albedo))} !Card 10c Tilt variables"
    )
    ...
    if "solar_zenith" in params and params["solar_zenith"] is not None:
        ...
        zenith, _, _ = calculate_solar_position(
...
def create_smarts_input(
    output_path,
    params,
    template_name="smarts_input",
    metadata_dir=None,
    ...
    if "solar_zenith" in params and params["solar_zenith"] is not None:
        ...
        zenith = min(
            80, zenith
        )  # Limit zenith angle to prevent extreme air mass values
    ...
    # Format exactly matching the PDF example - with Cards 10, 10b, and 10c on
    # separate lines
    content = [
    ...
    for param, (min_val, max_val) in range_checks.items():
        if param in params and params[param] is not None:
            if params[param] < min_val or params[param] > max_val:
                messages.append(
                    f"Parameter {param} value {params[param]} outside valid range [{min_val}, {max_val}]"
                )
    ...
    if remaining > 0:
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                if any(
                    abs(loc[1] - lat_point) < 0.5 and abs(loc[2] - lon_point) < 0.5
                    for loc in locations
                ):
                    continue

                # Determine land cover based on geographic position
                # Using a more deterministic approach instead of random
                # assignment
                if lat_point > 60:
                    # Northern regions: more likely to be conifer forests or
                    # snow
                ...
                if elevation_df is not None:
                    elevation = get_elevation_for_location(
                        elevation_df, lat_point, lon_point
                    )

                # Use reasonable defaults based on land cover if no elevation
                # data
                if elevation is None:
                    if land_cover == "SNOW":
                        elevation = 1500
...
def batch_process_locations(
    locations, days, time_points, ds, args, stats, elevation_df=None
):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate SMARTS input files from ERA5 data for EU locations"
    )
    ...
    parser.add_argument(
        "--elevation-file", help="CSV file with elevation data (optional)"
    )
    parser.add_argument("--metadata", help="Directory to save metadata JSON files")
    parser.add_argument(
        "--days", help="Comma-separated list of days (e.g., 2023-01-15,2023-06-21)"
    )
    parser.add_argument(
        "--times", help="Comma-separated list of time points (morning,noon,evening)"
    )
    parser.add_argument(
        "--limit", type=int, default=300, help="Maximum number of locations to process"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        "--db-table", default="locations", help="Table name for locations"
    )
    ...
    if args.dry_run:
        ...
        if args.days:
            try:
                test_days = [
                    pd.to_datetime(day.strip()) for day in args.days.split(",")
                ]
        ...
        try:
            ...
            for _, row in locations_df.iterrows():
                try:
                    ...
                    land_cover = (
                        row["land_cover"].upper()
                        if "land_cover" in row and not pd.isna(row["land_cover"])
                        else "LIGHT_SOIL"
                    )

                    # If no elevation in locations file, try to get from
                    # elevation data
    ...
    if args.days:
        days = [
            {"date": day.strip(), "description": day.strip()}
    ...
    stats["total_files"] = (
        stats["total_locations"] * stats["total_days"] * stats["total_time_points"]
    )
    ...
    try:
        ...
        if args.days:
            ...
            logger.info(
                f"Filtering dataset to time range: {min_date.date()} to {max_date.date()}"
            )
            ds = ds.sel(time=slice(min_date, max_date))

        # Process locations and times
        success_count, error_count, skipped_count, successful_files, failed_list = (
    ...
    logger.info(
        f"Successfully generated: {stats['successful']} ({stats['successful']/stats['total_files']*100:.1f}%)"
    )
    logger.info(
        f"Errors: {stats['errors']} ({stats['errors']/stats['total_files']*100:.1f}%)"
    )
    logger.info(f"Skipped (resume mode): {stats['skipped']}")
    if stats.get("graceful_degradation", 0) > 0:
        logger.info(
            f"Files created with graceful degradation: {stats['graceful_degradation']}"
        )
    logger.info(f"Processing time: {stats['processing_time']:.1f} seconds")
    logger.info(
        f"Average time per file: {stats['processing_time']/max(1, stats['successful'] + stats['errors']):.2f} seconds"
    )
    ...
    if stats["missing_critical"]:
        ...
        for param, count in sorted(
            stats["missing_critical"].items(), key=lambda x: x[1], reverse=True
        )[:5]:
    ...
    if stats["extraction_errors"]:
        ...
        for var, count in sorted(
            stats["extraction_errors"].items(), key=lambda x: x[1], reverse=True
        )[:5]:
            logger.info(f"  {var}: {count} errors")

    if stats["time_warnings"] > 0:
        logger.warning(
            f"Time index warnings: {stats['time_warnings']} (requested times not closely matched in ERA5 data)"
        )
