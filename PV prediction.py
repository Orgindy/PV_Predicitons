...
def train_ensemble_model(
    df, feature_cols, target_col="PV_Potential_physics", test_size=0.2, random_state=42
):
    """
...
def match_technology_to_clusters(cluster_spectra_df, pv_profiles, temp_col="T_air"):
    ...
    for _, row in cluster_spectra_df.iterrows():
        ...
        for tech, profile in pv_profiles.items():
            score = sum(
                row[band + "_band"] * profile["spectral_response"][band]
        ...
        results.append(
            {"Cluster_ID": cluster_id, "Best_Technology": best_match, **tech_scores}
        )
...
def main_clustering_pipeline(
    input_file="merged_dataset.csv", output_dir="results", n_clusters=5
):
    ...
    metrics_path = os.path.join(
        cluster_dir, f"model_metrics_k{n_clusters}_{run_id}.txt"
    )
    ...
    clustering_features = [
        ...
        "IR_band",
        "Total_band",
        "Predicted_PV_Potential",
    ]
    X_cluster_scaled, valid_idx = prepare_features_for_clustering(
        df_with_pred, clustering_features
    )
...
def multi_year_clustering(
    ...
    for file in input_files:
        ...
        output_dir / f"clustered_dataset_{year}.csv"
        matched_out = output_dir / f"matched_dataset_{year}.csv"

        # Run core clustering logic
        main_clustering_pipeline(
            input_file=str(file), output_dir=str(output_dir), n_clusters=n_clusters
        )
...
def generate_cluster_summaries(clustered_df, cluster_col="Cluster_ID", save_path=None):
    ...
    if "Best_Technology" in clustered_df.columns:
        top_tech = (
            clustered_df.groupby(cluster_col)["Best_Technology"]
...
def summarize_and_plot_multi_year_clusters(summary_df, output_dir="results/clusters"):
    ...
    tech_freq = (
        summary_df.groupby(["Year", "Best_Technology"]).size().reset_index(name="Count")
    )
    plt.figure(figsize=(10, 6))
    ...
    avg_scores = (
        summary_df.groupby(["Year", "Best_Technology"])
...
if __name__ == "__main__":
    ...
    if args.db_url:
        ...
        write_dataframe(
            df_clustered, args.db_table, db_url=args.db_url, if_exists="replace"
        )
    else:
