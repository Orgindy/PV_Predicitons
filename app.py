...
def estimate_pv_cell_temperature(ghi, tair_c, wind, model="NOCT"):
    ...
    if model == "NOCT":
        ...
        wind_cooling = (
            np.clip(wind - 1, 0, 10) * 0.5
        )  # 0.5°C cooling per m/s above 1 m/s
...
def load_cluster_dataset(csv_path=DATA_PATH, db_url=DB_URL, db_table=DB_TABLE):
    ...
    try:
        ...
        df = df.rename(
            columns={coordinate_mapping["lat"]: "lat", coordinate_mapping["lon"]: "lon"}
        )
...
with st.sidebar:
    ...
    num_neighbors = st.slider(
        "Number of Similar Zones", 1, 10, 5, help="Number of similar zones to recommend"
    )
        ...
        if st.button("🚀 Run Performance Simulation", type="primary"):
            with st.spinner("Running simulation..."):
                ...
                with match_col1:
                    ...
                    st.metric("Cluster ID", match.get("Cluster_ID", "N/A"))
                ...
                with match_col2:
                    ...
                    rc_val = match.get(
                        "RC_Potential",
                        match.get("RC_potential", match.get("rc_potential", 0)),
                    )
                ...
                with match_col3:
                    ...
                    if "Synergy_Index" in match:
                        st.metric(
                            "Historical Synergy", f"{match['Synergy_Index']:.1f}%"
                        )
                ...
                T_pv_c = estimate_pv_cell_temperature(
                    ghi_profile, temp_profile_c, wind_profile, model=pv_model
                )
                T_rc_c = T_rc_k - 273.15

                # Calculate synergy index
                synergy_index = calculate_synergy_index(
                    T_pv_c, T_rc_c, ghi_profile, gamma_pv=gamma
                )
                ...
                if show_advanced:
                    ...
                    with adv_col1:
                        ...
                        st.write(
                            f"• Temperature Variance: {np.var(T_pv_c - T_rc_c):.2f}"
                        )
                ...
                if cluster_id != 0 and "Cluster_ID" in df.columns:
                    ...
                    if len(similar_zones) > 1:
                        # Calculate distance from user location to all similar
                        # zones
                        ...
                        if sort_col == "distance_to_user":
                            similar_zones = similar_zones.sort_values(
                        ...
                        for idx, (_, zone) in enumerate(top_zones.iterrows()):
                            ...
                            with col_c:
                                rc_val = zone.get(
                                    "RC_Potential", zone.get("RC_potential", 0)
                                )
                    ...
                    st.info(
                        "Cluster information not available for zone recommendations."
                    )
...
"""
        ...
        st.error(
            "❌ Cannot load dataset. Please ensure the required files are available."
        )
    ...
    if df is not None:
        ...
        with st.expander("📊 Dataset Statistics"):
            ...
            if "lat" in df.columns and "lon" in df.columns:
                ...
                st.write(
                    f"• Latitude: {df['lat'].min():.1f}° to {df['lat'].max():.1f}°"
                )
                st.write(
                    f"• Longitude: {df['lon'].min():.1f}° to {df['lon'].max():.1f}°"
                )
