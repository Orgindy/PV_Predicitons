...
def main():
    ...
    if len(feature_columns) < 3:
        print(
            f"❌ Not enough features found ({len(feature_columns)}). Need at least 3 for clustering."
        )
        return
    ...
    try:
        scores.append(
            apply_and_store(df, X, "dbscan", run_dbscan, eps=0.5, min_samples=5)
        )
    except Exception as e:
    ...
    try:
        scores.append(
            apply_and_store(df, X, "agglomerative", run_agglomerative, n_clusters=5)
        )
    except Exception as e:
