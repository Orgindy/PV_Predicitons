    ...
    try:
        ...
        if spatial_subset:
            ...
            if "latitude" in ds.dims and "longitude" in ds.dims:
                ...
                logger.info(
                    f"Spatial subset applied: lat [{lat_min}, {lat_max}], lon [{lon_min}, {lon_max}]"
                )
...
def detect_time_step(ds: xr.Dataset) -> int:
    ...
    if len(ds.time) > 1:
        ...
        if len(time_steps_seconds) > 0 and np.all(
            time_steps_seconds == time_steps_seconds[0]
        ):
            ...
            logger.warning(
                f"Variable time steps detected. Using most common: {time_step_seconds} seconds"
            )
    else:
...
def convert_kelvin_to_celsius(da: xr.DataArray) -> xr.DataArray:
    ...
    if da.min() < 100:
        logger.info(
            "Temperature appears to be already in Celsius, no conversion needed"
        )
        result = da.copy()
    else:
...
def calculate_relative_humidity(
    temp_c: xr.DataArray, dewp_c: xr.DataArray
) -> xr.DataArray:
    """
...
def calculate_derived_variables(ds: xr.Dataset, time_step_seconds: int) -> xr.Dataset:
    ...
    if "t2m" in ds:
        ...
        if hasattr(ds.t2m, "units") and ds.t2m.units == "K":
            ...
            if ds.t2m.mean() > 100:
                ds_derived["T2m"] = ds.t2m - 273.15
                logger.info(
                    "Guessed t2m is in Kelvin based on values, converted to Celsius"
                )
            else:
    ...
    if "T2m" in ds_derived and "Td2m" in ds_derived:
        ...
        # Ensure dewpoint <= temperature (physical constraint)
        dewp_c = xr.where(
            ds_derived.Td2m > ds_derived.T2m, ds_derived.T2m, ds_derived.Td2m
        )
    ...
    if "tp" in ds:
        ...
        if hasattr(ds.tp, "units") and ds.tp.units in ["m", "metres"]:
            ...
            if ds.tp.mean() < 0.1:
                ds_derived["Precip"] = ds.tp * 1000
                logger.info(
                    "Guessed precipitation is in m based on values, converted to mm"
                )
            else:
    ...
    ds_derived.attrs["processing_history"] = (
        f"Derived variables created at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
def calculate_radiation_variables(ds: xr.Dataset, time_step_seconds: int) -> xr.Dataset:
    ...
    for var in rad_vars:
        if var in ds:
            ...
            ds_rad[var_name_energy].attrs = (
                ds[var].attrs.copy() if hasattr(ds[var], "attrs") else {}
            )
...
def aggregate_to_daily(ds: xr.Dataset) -> xr.Dataset:
    ...
    ds_daily = xr.Dataset(
        coords={"time": time_range, "latitude": ds.latitude, "longitude": ds.longitude}
    )
    ...
    if use_tqdm and mean_vars:
        batch_iterator = tqdm(
            range(0, len(mean_vars), batch_size), desc="Mean variables"
        )
    else:
    ...
    if max_vars:
        ...
        if use_tqdm:
            batch_iterator = tqdm(
                range(0, len(max_vars), batch_size), desc="Max variables"
            )
        else:
    ...
    if min_vars:
        ...
        if use_tqdm:
            batch_iterator = tqdm(
                range(0, len(min_vars), batch_size), desc="Min variables"
            )
        else:
    ...
    ds_daily.attrs["processing_history"] = (
        ds.attrs.get("processing_history", "")
        + f"; Daily aggregation at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )

    logger.info(
        f"Daily aggregation completed in {time.time() - start_time:.2f} seconds"
    )
    return ds_daily
...
def aggregate_to_monthly(ds_daily: xr.Dataset) -> xr.Dataset:
    ...
    ds_monthly = xr.Dataset(
        coords={
            "time": pd.date_range(
                start=start_year_month, end=end_year_month, freq="MS"  # Month start
            ),
    ...
    for var_name in ds_daily.data_vars:
        ...
        try:
            ...
            if method == "sum":
                ds_monthly[var_name] = (
                    ds_daily[var_name].resample(time="MS").sum(skipna=True)
                )
            else:  # mean
                ds_monthly[var_name] = (
                    ds_daily[var_name].resample(time="MS").mean(skipna=True)
                )
    ...
    ds_monthly.attrs["processing_history"] = (
        ds_daily.attrs.get("processing_history", "")
        + f"; Monthly aggregation at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
def aggregate_to_yearly(ds_monthly: xr.Dataset) -> xr.Dataset:
    ...
    for var_name in ds_monthly.data_vars:
        ...
        try:
            ...
            if method == "sum":
                ds_yearly[var_name] = (
                    ds_monthly[var_name].resample(time="YS").sum(skipna=True)
                )
            else:  # mean
                ds_yearly[var_name] = (
                    ds_monthly[var_name].resample(time="YS").mean(skipna=True)
                )
    ...
    ds_yearly.attrs["processing_history"] = (
        ds_monthly.attrs.get("processing_history", "")
        + f"; Yearly aggregation at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
def calculate_degree_days(ds_daily: xr.Dataset) -> xr.Dataset:
    ...
    ds_daily["HDD"] = xr.where(
        ds_daily.T2m < base_temp_hdd, base_temp_hdd - ds_daily.T2m, 0
    ).clip(min=0)
    ...
    ds_daily["CDD"] = xr.where(
        ds_daily.T2m > base_temp_cdd, ds_daily.T2m - base_temp_cdd, 0
    ).clip(min=0)
    ...
    if "Tmax" in ds_daily and "Tmin" in ds_daily:
        ...
        ds_daily["GDD"] = (((ds_daily.Tmax + ds_daily.Tmin) / 2) - base_temp_gdd).clip(
            min=0
        )
...
def calculate_consecutive_dry_days(ds_daily: xr.Dataset) -> xr.Dataset:
    ...
    if "Precip" not in ds_daily:
        logger.warning(
            "Precipitation data not available for consecutive dry days calculation"
        )
        return xr.Dataset()
    ...
    ds_yearly = xr.Dataset(
        coords={
            "time": pd.date_range(
                start=f"{unique_years[0]}-01-01", periods=len(unique_years), freq="YS"
            ),
            "latitude": ds_daily.latitude,
            "longitude": ds_daily.longitude,
        }
    ...
    try:
        ...
        for year in year_iter:
            ...
            cdd_year = cdd_year.expand_dims(time=[pd.Timestamp(f"{year}-01-01")])
        ...
        logger.info(
            f"Consecutive dry days calculation completed in {time.time() - start_time:.2f} seconds"
        )
        return ds_yearly
...
def aggregate_to_daily_day_and_night(ds):
    ...
    ds_daily = ds_daily.rename(
        {"P_rc_net": "RC_total", "P_rc_day": "RC_day", "P_rc_night": "RC_night"}
    )
...
def process_era5(
    ...
    try:
        ...
        logger.info(
            f"Derived variables calculated in {time.time() - step_start:.2f} seconds"
        )
        ...
        logger.info(
            f"Daily aggregation completed in {time.time() - step_start:.2f} seconds"
        )
        ...
        logger.info(
            f"Monthly aggregation completed in {time.time() - step_start:.2f} seconds"
        )
        ...
        logger.info(
            f"Yearly aggregation completed in {time.time() - step_start:.2f} seconds"
        )
        ...
        logger.info(
            f"Climate indices calculated in {time.time() - step_start:.2f} seconds"
        )
        ...
        logger.info(
            f"ERA5 processing completed successfully in {total_time:.2f} seconds ({total_time/60:.2f} minutes)"
        )
        return saved_files
