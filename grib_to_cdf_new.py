...
GRIB_FOLDER = os.getenv("GRIB_FOLDER", "grib_files")
NETCDF_OUTPUT_FOLDER = os.getenv("NETCDF_OUTPUT_FOLDER") or get_nc_dir()
MERGED_NETCDF_FILE = os.path.join(
    get_nc_dir(), os.getenv("MERGED_NETCDF_FILE", "era5_2023_merged.nc")
)
...
def grib_to_netcdf(input_file, output_folder):
    ...
    try:
        # Open the GRIB file using cfgrib engine with additional options for
...
def convert_all_grib_files(grib_folder, output_folder):
    ...
    logger.info(
        f"Successfully converted {len(successful_conversions)} out of {len(grib_files)} files"
    )
    ...
    try:
        ...
        for dim in dimensions_to_check:
            ...
            if all(dim in ds.dims for ds in sample_datasets):
                # Check if values are different across datasets (makes sense to
                # concatenate)
        ...
        logger.info(
            "No suitable dimension found for concatenation, will use coordinate-based merge"
        )
...
def merge_netcdf_files_chunked(
    netcdf_files, output_file, merge_dim=None, chunk_size=None
):
    """
    ...
    try:
        ...
        chunks = [
            netcdf_files[i : i + chunk_size]
            for i in range(0, len(netcdf_files), chunk_size)
        ]
        ...
        for i, chunk in enumerate(tqdm(chunks, desc="Merging chunks")):
            ...
            if merge_dim and all(merge_dim in ds.dims for ds in datasets):
                ...
                logger.debug(
                    f"Merged chunk {i+1}/{len(chunks)} along {merge_dim} dimension"
                )
            else:
                # Fall back to coordinate-based merge
                merged_ds = xr.merge(datasets)
                logger.debug(
                    f"Merged chunk {i+1}/{len(chunks)} using coordinate-based merge"
                )
            ...
            merged_ds.to_netcdf(
                temp_file,
                encoding={
                    var: {"zlib": True, "complevel": 5} for var in merged_ds.data_vars
                },
            )
