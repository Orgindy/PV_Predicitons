...
def extract_metadata(out_file_content):
    ...
    temp_match = re.search(
        r"Instantaneous at site\'s altitude = (\d+\.\d+)", out_file_content
    )
    if temp_match:
    ...
    zenith_match = re.search(
        r"Zenith Angle \(apparent\) = (\d+\.\d+)", out_file_content
    )
    if zenith_match:
...
def parse_ext_file(ext_file_content):
    ...
    # Convert wavelength from nanometers to micrometers for easier band
    # definition
...
def define_spectral_bands(df):
    ...
    for band_name, (wl_min, wl_max) in bands.items():
        band_data = df[
            (df["wavelength_um"] >= wl_min) & (df["wavelength_um"] <= wl_max)
        ]
        ...
        # Calculate integrated values for each column of interest using
        # trapezoidal integration
        band_irradiance[band_name] = {
            "Global_tilted_irradiance": np.trapz(
                band_data["Global_tilted_irradiance"], band_data["wavelength_um"]
            ),
            "Beam_normal_+circumsolar": np.trapz(
                band_data["Beam_normal_+circumsolar"], band_data["wavelength_um"]
            ),
            "Difuse_horiz-circumsolar": np.trapz(
                band_data["Difuse_horiz-circumsolar"], band_data["wavelength_um"]
            ),
            "Zonal_ground_reflectance": np.mean(
                band_data["Zonal_ground_reflectance"]
            ),  # Mean for reflectance
            "wavelength_range": f"{wl_min}-{wl_max} µm",
...
def save_to_netcdf(metadata, band_irradiance, spectral_df, output_dir, filename_base):
    ...
    with nc.Dataset(output_path, "w", format="NETCDF4") as ncfile:
        ...
        diffuse_var = ncfile.createVariable(
            "diffuse_horizontal_irradiance", "f4", ("band",)
        )
        reflectance_var = ncfile.createVariable(
            "average_ground_reflectance", "f4", ("band",)
        )
        ...
        global_var[:] = [
            band_irradiance[band]["Global_tilted_irradiance"] for band in band_names
        ]
        beam_var[:] = [
            band_irradiance[band]["Beam_normal_+circumsolar"] for band in band_names
        ]
        diffuse_var[:] = [
            band_irradiance[band]["Difuse_horiz-circumsolar"] for band in band_names
        ]
        reflectance_var[:] = [
            band_irradiance[band]["Zonal_ground_reflectance"] for band in band_names
        ]
...
def process_directory(input_dir, output_dir):
    ...
    for out_file, ext_file in file_pairs:
        print(
            f"Processing {os.path.basename(out_file)} and {os.path.basename(ext_file)}..."
        )
        ...
        # Process the files
        metadata, band_irradiance, spectral_df = process_smarts_files(
            out_file, ext_file
        )

        # Save to CSV and NetCDF
        csv_path = save_to_csv(metadata, band_irradiance, output_dir, base_name)
        nc_path = save_to_netcdf(
            metadata, band_irradiance, spectral_df, output_dir, base_name
        )
